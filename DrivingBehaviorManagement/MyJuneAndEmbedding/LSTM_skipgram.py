...
#encoding
#npy的读取
vec = np.load('embedding.npy')
# np.savetxt( "embedding.csv", vec, delimiter="," )
pattern_onehot = np.empty((len(pattern), maxlen, 10))
for i in range(len(pattern)):
    pattern_encode = np.zeros((maxlen,10))
    for j in range(len(pattern[i])):
        encode = vec[all_labels.index(pattern[i][j])]
...
plt.plot(inv_yhat, c='orange', label='预测数据-embedding')

inv_yhat_onehot = np.load('yhat_onehot.npy')
plt.plot(inv_yhat_onehot, c='salmon', label='预测数据-onehot', linestyle='--')
