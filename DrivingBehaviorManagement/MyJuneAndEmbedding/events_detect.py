...
class EVENT:
    ...
    def accel_eventresult(self, start, end):
        ...
        # 速度标准差
        spd_std = np.std(self.spdsave, ddof=1)
        self.spdstd.append(spd_std)
        # 速度均值
        spd_mean = np.mean(self.spdsave)
        self.spdmea.append(spd_mean)
        # 最大加速度
        ...
        if spd_mean == 0:
            ...
            self.spdcv.append(spd_std / spd_mean)
        # self.spdmad.append()
        # self.amad.append()
        spdsave_len = len(self.spdsave)
        vfall = []
        if spdsave_len == 1:
            self.spdvf.append(0)
        if spdsave_len == 2:
            # if 0 in self.spdsave:
            self.spdvf.append(0)
            # else:
            #     self.spdvf.append(np.log(self.spdsave[1])-np.log(self.spdsave[0]))
        if spdsave_len == 3:
            # if 0 in self.spdsave:
            #     self.spdvf.append(0)
            # else:
            #     r2 = np.log(self.spdsave[2]) - np.log(self.spdsave[1])
            #     r1 = np.log(self.spdsave[1]) - np.log(self.spdsave[0])
            #     rall = [r2, r1]
            #     self.spdvf.append(np.std(rall, ddof=1))
            self.spdvf.append(0)
        ...
        if spdsave_len > 3:
            if 0 in self.spdsave:
                ...
                start = 1
                for i in range(1, spdsave_len - 3 + 1):
                    r3 = np.log(self.spdsave[start + 2]) - np.log(self.spdsave[start + 1])
                    r2 = np.log(self.spdsave[start + 1]) - np.log(self.spdsave[start])
                    r1 = np.log(self.spdsave[start]) - np.log(self.spdsave[start - 1])
                    rall = [r3, r2, r1]
                    vfall.append(np.std(rall, ddof=1))
                    start += 1
                self.spdvf.append(np.mean(vfall))
    ...
    def turn_eventresult(self, start, end):
        ...
        # 加速度取绝对值，我认为只需要表示速度变化趋势的大小。（加速过弯应该很罕见吧。。）
        absolute_a = np.maximum(np.array(self.accelsave), -np.array(self.accelsave))
        # 最大加速度
